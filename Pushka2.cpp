...
bool Pushka::fire(Yadro * yadro, int i, Pushka * gun,int lengt,int height)
{
	if (isShoot)
	{
		time += 0.001;
		...
		if ((yadro->get_x() > lengt || yadro->get_x() < 0) && (yadro->get_y() >height))// check for a kernel out of the window
		{
			isShoot = false;
			time = 0;
